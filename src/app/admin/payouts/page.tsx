...
function CommissionBreakdown({ amount }: { amount: number }) {
  ...
  return (
    <div className="grid grid-cols-3 gap-2 text-xs mt-3 pt-3 border-t border-stone-100">
      <div>
        <p className="text-stone-400 mb-0.5">Ordertotal</p>
        <p className="font-semibold text-slate-800">{formatAmount(amount)}</p>
      </div>
      <div>
        <p className="text-stone-400 mb-0.5">Provision (5%)</p>
        <p className="font-semibold text-red-600">−{formatAmount(commission)}</p>
      </div>
      <div>
        <p className="text-stone-400 mb-0.5">Till säljaren</p>
      ...
      <div className="flex items-start gap-4 p-5">
        ...
        <div className="flex-1 min-w-0 space-y-1">
          <div className="flex items-start justify-between gap-2">
            <div>
              <p className="font-semibold text-slate-900 truncate">{order.artwork?.title ?? 'Okänt konstverk'}</p>
          ...
          <div className="grid grid-cols-2 gap-x-4 text-sm pt-1">
            ...
            <div>
              <p className="text-xs text-slate-400 uppercase tracking-wide font-medium">Säljare</p>
          ...
          {onMethodChange && method && (
            <div className="flex items-center gap-4 pt-3 text-sm">
              <span className="text-xs text-stone-500 font-medium">Betala via:</span>
              ...
              <label className="flex items-center gap-1.5 cursor-pointer">
                <input
                  ...
                  className="accent-green-600"
                />
                <span className="text-slate-700">Banköverföring</span>
              </label>
            </div>
          )}
          ...
          <div className="flex items-center gap-3 pt-3">
            {showNote && (
              <input
                type="text"
                placeholder="Anteckning (valfri, t.ex. Swish-kvitto)"
                value={note}
      ...
      <div className="flex items-start gap-4 p-5">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="grid grid-cols-3 gap-2 text-xs mt-3 pt-3 border-t border-stone-100">
            ...
            <div>
              <p className="text-stone-400 mb-0.5">Provision</p>
              <p className="font-semibold text-red-600">−{formatAmount(payout.commissionAmount)}</p>
            </div>
            <div>
              <p className="text-stone-400 mb-0.5">Utbetalat</p>
          ...
          <div className="flex items-center gap-3 mt-2 text-xs text-stone-400">
            ...
            <span>·</span>
            <span>{payout.method === 'bank' ? 'Banköverföring' : 'Swish'}</span>
            {payout.completedAt && (
              <>
                <span>·</span>
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-10 space-y-10">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Betalningar & utbetalningar</h1>
          <p className="text-sm text-slate-500 mt-1">Hantera Swish-bekräftelser och säljarutbetalningar</p>
        </div>
        <Link href="/admin" className="text-sm text-slate-500 hover:text-slate-900 transition-colors">← Admin</Link>
      </div>
      ...
      {activeTab === 'pending' && (
        <>
          {loading && <div className="py-20 text-center text-slate-400 text-sm">Laddar...</div>}
          {error && <div className="bg-red-50 border border-red-200 rounded-xl p-4 text-red-700 text-sm">{error}</div>}
          {!loading && !error && isEmpty && (
            <div className="py-20 text-center space-y-2">
              <div className="text-3xl">✓</div>
              <p className="text-slate-500">Allt är klart — inga väntande åtgärder</p>
            </div>
          )}
          ...
          {pendingSwish.length > 0 && (
            <section className="space-y-4">
              <div className="flex items-center gap-3">
                ...
                <h2 className="font-semibold text-slate-900">Swish — väntar på betalningsbekräftelse</h2>
                <span className="ml-auto text-xs text-slate-400">{pendingSwish.length} order</span>
              </div>
              <p className="text-xs text-slate-500 -mt-1">
                Kontrollera att Swish-betalningen kommit in, klicka sedan "Bekräfta" — köparen och säljaren får bekräftelsemail.
              </p>
              {pendingSwish.map((order) => (
                <OrderCard
                  key={order.id}
                  order={order}
                  actionLabel="Bekräfta betalning"
          ...
          {unpaidPayouts.length > 0 && (
            <section className="space-y-4">
              <div className="flex items-center gap-3">
                ...
                <h2 className="font-semibold text-slate-900">Utbetalningar till säljare</h2>
                <span className="ml-auto text-xs text-slate-400">{unpaidPayouts.length} order</span>
              </div>

              <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex items-center justify-between">
                <div>
                  <p className="text-sm text-amber-700 font-medium">Totalt att betala ut (efter provision)</p>
                  <p className="text-2xl font-bold text-amber-900">{formatAmount(totalPayout)}</p>
                </div>
                <p className="text-xs text-amber-600 text-right">Betala ut till varje säljare separat</p>
              </div>
      ...
      {activeTab === 'completed' && (
        <section className="space-y-4">
          {completedLoading && <div className="py-10 text-center text-slate-400 text-sm">Laddar...</div>}
          {!completedLoading && completedPayouts.length === 0 && (
            <div className="py-20 text-center space-y-2">
              <p className="text-slate-400 text-sm">Inga genomförda utbetalningar ännu.</p>
            </div>
          )}
