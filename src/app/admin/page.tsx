  ...
  const [users, artworks, challenges, resendHealth] = await Promise.all([
    ...
    fetch(`${appUrl}/api/health/resend`, { cache: 'no-store' })
      ...
      .catch((error) => ({
        ok: false,
        status: 500,
        message: error instanceof Error ? error.message : 'Kunde inte läsa Resend-status.'
      }))
  ]);
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-md border border-slate-200 p-4">
        <h2 className="text-lg font-medium">Resend hälsa</h2>
        <p className={`mt-2 text-sm ${resendHealth.ok ? 'text-emerald-700' : 'text-red-700'}`}>
          Status: {resendHealth.ok ? 'OK' : 'Fel'} (HTTP {resendHealth.status})
        </p>
        <p className="mt-1 text-sm text-slate-600">{resendHealth.message || 'Ingen statusmessage.'}</p>
      </section>
      <section>
