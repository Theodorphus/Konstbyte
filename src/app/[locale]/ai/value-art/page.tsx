...
function TipsBox({ tips, title }: { tips: string[]; title: string }) {
  return (
    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 503) {
          ...
          setError(data.error || t("error_valuation_generic"));
        }
      } else {
        setResult(data.valuation || t("no_valuation"));
      }
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-8">
      ...
      <div className="rounded-2xl bg-gradient-to-br from-amber-500 to-orange-600 p-6 text-white space-y-2 shadow-md">
        ...
        <h1 className="text-2xl font-bold tracking-tight">{t("value_art_title")}</h1>
        <p className="text-sm text-amber-100 leading-relaxed">
          {t("value_art_subtitle")}
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="rounded-2xl border border-amber-100 bg-white p-6 shadow-sm space-y-4">
          <SectionHeader
            ...
            title={t("image_section_title")}
            subtitle={t("image_section_sub")}
          />
          ...
          {!imageUrl ? (
            ...
            <div className="rounded-xl border-2 border-dashed border-amber-200 bg-stone-50 min-h-[220px] flex flex-col items-center justify-center gap-4 p-6 transition-colors hover:border-amber-300 hover:bg-amber-50/50">
              ...
              <div className="text-center space-y-1 w-full max-w-xs">
                ...
                <div className="flex items-center gap-2 my-2">
                  ...
                  <span className="text-xs text-slate-400">{t("or_upload")}</span>
                  <div className="flex-1 h-px bg-stone-200" />
                </div>
            ...
            <div className="space-y-3">
              ...
              {/* Actions below image */}
              <div className="flex items-center gap-3">
                <WallPreview artworkUrl={imageUrl} artworkTitle={t("your_artwork")} />
                <button
                  type="button"
                  onClick={() => setImageUrl("")}
        ...
        <div className="rounded-2xl border border-stone-200 bg-stone-50 p-6 shadow-sm space-y-5">
          ...
          <SectionHeader
            ...
            title={t("basic_title")}
            subtitle={t("basic_sub")}
          />
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <FieldInput label={t("artist_label")} value={artist} onChange={setArtist} placeholder={t("artist_placeholder")} />
            <FieldInput label={t("art_type_label")} value={artType} onChange={setArtType} placeholder={t("art_type_placeholder")} />
          </div>
          ...
          <SectionHeader
            ...
            title={t("technique_title")}
            subtitle={t("technique_sub")}
          />
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <FieldInput label={t("style_label")} value={style} onChange={setStyle} placeholder={t("style_placeholder")} />
            <FieldInput label={t("size_label")} value={sizeMaterial} onChange={setSizeMaterial} placeholder={t("size_placeholder")} />
          </div>
      ...
      {!result && !loading && (
        <div className="space-y-5">
            ...
            <div className="grid grid-cols-3 gap-3">
              {exampleImages.map((img, i) => (
                ...
                >
                  <Image
                    src={img}
                    alt={t("example_artwork_alt", { num: i + 1 })}
                    fill
          ...
          <TipsBox tips={tips} title={t("tips_title")} />
        </div>
      )}
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <p className="text-sm font-bold text-amber-900">{t("valuation_result")}</p>
                <p className="text-xs text-amber-600 mt-0.5">{t("by_ai")}</p>
              </div>
            </div>
            <Button
              size="sm"
