...
export default function EditArtworkPage() {
  const t = useTranslations('my_artworks');
  ...
  return (
    <div className="max-w-xl space-y-4">
      ...
      <form onSubmit={handleSave} className="space-y-3">
        ...
        <button type="submit" className="bg-slate-900 text-white px-4 py-2 rounded">{t('save')}</button>
        <button type="button" onClick={handleDelete} className="ml-2 text-red-600">{t('delete')}</button>
      </form>
    </div>
  );
}
