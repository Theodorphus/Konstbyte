...
export default function SignInPage() {
  ...
  return (
    <Suspense fallback={<div className="max-w-md mx-auto py-12 text-slate-600">{t('loading_signin')}</div>}>
      <SignInContent />
    </Suspense>
  );
}
...
function SignInContent() {
  ...
  const errorMessages: Record<string, string> = Object.fromEntries(
    ERROR_KEYS.map((k) => [k, t(`error_${k}` as `error_${ErrorKey}`)])
  );
  ...
  return (
    <div className="max-w-md mx-auto py-12">
      <h1 className="text-2xl font-semibold">{t('sign_in')}</h1>
      <p className="text-slate-600 mt-2">{t('signin_subtitle')}</p>

      {errorMessage ? (
        <div className="mt-4 rounded-md border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-800">
          {errorMessage}
          {errorCode ? ` ${t('error_code', { code: errorCode })}` : ''}
        </div>
      ) : null}
      ...
      <div className="mt-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div className="text-sm text-slate-500">{t('or')}</div>
          <div className="flex-1 h-px bg-slate-200" />
        </div>
        <Button
