...
function timeAgo(dateStr: string, t: (key: string, values?: Record<string, number>) => string) {
  ...
  return t('days_ago', { count: Math.floor(diff / 86400) });
}
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div className="space-y-1">
        ...
        <p className="text-sm text-slate-500">{t('conversations_desc')}</p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="rounded-2xl border border-slate-100 bg-slate-50 p-10 text-center space-y-2">
          ...
          <p className="font-medium text-slate-700">{t('no_conversations')}</p>
          <p className="text-sm text-slate-400">{t('no_conversations_sub')}</p>
        </div>
      ) : (
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-baseline justify-between gap-2">
                    ...
                    <span className="text-xs text-slate-400 flex-shrink-0">
                      {timeAgo(lastMessage.createdAt, t)}
                    </span>
                  </div>
